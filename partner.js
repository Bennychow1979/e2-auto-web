import {showroomLink} from './showroom-referral.mjs';
...
async function load(){
  ...
  try{
    ...
    $('showroomLink').value=showroomLink(location.href,partner.code);
...
$('copyShowroom').onclick=async()=>{try{await navigator.clipboard.writeText($('showroomLink').value);$('message').textContent='Showroom link copied.'}catch{$('showroomLink').select();$('message').textContent='Copy the selected showroom link.'}};
