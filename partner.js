...
$('login').onsubmit=async e=>{e.preventDefault();try{const form=new FormData(e.target);check(await db.auth.signInWithPassword({email:form.get('email'),password:form.get('password')}));e.target.reset();await load()}catch(error){$('message').textContent=error.message}};
