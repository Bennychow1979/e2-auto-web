...
$('bookingForm').onsubmit=e=>{e.preventDefault();const date=$('viewDate').value;if(!date||date<malaysiaDate()){$('bookingError').hidden=false;return}$('bookingLink').href=wa(intro+'I would like a viewing of this vehicle. Preferred date: '+date+', '+$('viewTime').value+' (Malaysia time). Please confirm availability and the viewing time at Centre Point Batu Caves, LG Basement Parking.');$('bookingReady').hidden=false;$('bookingForm').querySelector('button[type=submit]').hidden=true;$('bookingLink').focus()};
