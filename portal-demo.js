...
const seed=()=>({
cars:[
{id:'c1',mileage:72000,brand:'Toyota',model:'RAV4',spec:'2.0 — sample spec',cc:2000,fuel:'PETROL',transmission:'Auto',plate:'DEMO01',year:2018,price:89900,status:'Available',img:'https://images.unsplash.com/photo-1615887110697-0819ec23465f?auto=format&fit=crop&w=800&q=80'},
{id:'c2',mileage:32000,brand:'Polestar',model:'2',spec:'Electric — sample spec',cc:0,fuel:'ELECTRIC',transmission:'Single-speed (EV)',plate:'DEMO02',year:2022,price:168000,status:'Reserved',img:'https://images.unsplash.com/photo-1617727553401-3ec4e92f32a5?auto=format&fit=crop&w=800&q=80'},
{id:'c3',mileage:58000,brand:'BMW',model:'M4',spec:'3.0 — sample spec',cc:3000,fuel:'PETROL',transmission:'DCT',plate:'DEMO03',year:2019,price:238000,status:'Available',img:'https://images.unsplash.com/photo-1614026480209-cd9934144671?auto=format&fit=crop&w=800&q=80'}],
leads:[
...
if(kind==='car'){obj=data.cars.find(c=>c.id===id)||{plate:'',brand:'',model:'',spec:'',cc:'',mileage:null,fuel:'',transmission:'',year:2022,price:0,status:'Available'};title=id?'Edit sample vehicle':'Add sample vehicle';fields=field('CAR PLATE','plate',obj.plate)+identityField('brand')+identityField('model')+identityField('spec')+'<p class="field full identityHelp">Choose brand → model → variant (SPEC). Options come from the sample catalogue and vehicles saved in this demo. Missing a choice? Select Other / enter manually.</p>'+'<label class="field">ENGINE SIZE (L)<input name="engineLitres" type="number" inputmode="decimal" min="0" max="20" step="0.1" placeholder="1.0 / 1.5 / 2.0" value="'+(obj.cc===''?'':(Number(obj.cc)/1000).toFixed(1))+'" required></label>'+select('TRANSMISSION','transmission',obj.transmission,[{value:'',label:'Select transmission'},...transmissions])+select('FUEL TYPE','fuel',obj.fuel,[{value:'',label:'Select fuel type'},...fuelTypes],true)+'<p class="field full" style="color:#6b6f76;line-height:1.6;margin:0">Enter litres, e.g. 1.0, 1.5 or 2.0. Fully electric vehicles: 0.0. All demo specifications are illustrative.</p>'+field('Year','year',obj.year,'number')+'<label class="field">MILEAGE (km)<input name="mileage" type="number" inputmode="numeric" min="0" step="1" placeholder="e.g. 75000" value="'+(obj.mileage==null?'':obj.mileage)+'"><small>Leave blank if pending confirmation.</small></label>'+field('Illustrative price (RM)','price',obj.price,'number')+select('Status','status',obj.status,['Available','Reserved','Sold'],true);}
...
if(edit.kind==='car'){if(role!=='admin')return;if(!['plate','brand','model','spec','engineLitres','transmission','fuel'].every(k=>f[k]!==undefined&&f[k]!=='')){notify('Complete all required vehicle fields.');return}f.plate=f.plate.replace(/\s+/g,'').toUpperCase();if(data.cars.some(c=>c.plate.replace(/\s+/g,'').toUpperCase()===f.plate&&c.id!==id)){notify('That car plate already exists. Use a unique plate.');return}const litres=Number(f.engineLitres);if(!Number.isFinite(litres)||litres<0||litres>20||Math.abs(litres*10-Math.round(litres*10))>1e-7){notify('Enter engine size in litres, e.g. 1.0, 1.5 or 2.0 (maximum 20.0).');return}if(!transmissions.includes(f.transmission)||!fuelTypes.includes(f.fuel)){notify('Select a transmission and fuel type.');return}f.cc=Math.round(litres*1000);delete f.engineLitres;f.year=Number(f.year);f.price=Number(f.price);if(!Number.isInteger(f.year)||f.year<1900||f.year>2100||!Number.isFinite(f.price)||f.price<=0){notify('Use a year from 1900–2100 and a price above RM0.');return}f.mileage=f.mileage===''||f.mileage===undefined?null:Number(f.mileage);if(f.mileage!==null&&(!Number.isSafeInteger(f.mileage)||f.mileage<0)){notify('Enter mileage as a non-negative whole number in km, or leave it blank if unconfirmed.');return}f.media=mediaDraft.map(m=>({...m}));f.img=f.media.find(m=>m.type==='photo')?.url||'';const idx=data.cars.findIndex(c=>c.id===id);if(idx<0)data.cars.push({id,...f});else data.cars[idx]={...data.cars[idx],...f};}
