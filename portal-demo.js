...
if(edit.kind==='car'){if(role!=='admin')return;if(!['plate','brand','model','spec','engineLitres','transmission','fuel'].every(k=>f[k]!==undefined&&f[k]!=='')){notify('Complete all required vehicle fields.');return}f.plate=f.plate.replace(/\s+/g,'').toUpperCase();if(data.cars.some(c=>c.plate.replace(/\s+/g,'').toUpperCase()===f.plate&&c.id!==id)){notify('That car plate already exists. Use a unique plate.');return}const litres=Number(f.engineLitres);if(!Number.isFinite(litres)||litres<0||litres>20||Math.abs(litres*10-Math.round(litres*10))>1e-7){notify('Enter engine size in litres, e.g. 1.0, 1.5 or 2.0 (maximum 20.0).');return}if(!transmissions.includes(f.transmission)||!fuelTypes.includes(f.fuel)){notify('Select a transmission and fuel type.');return}f.cc=Math.round(litres*1000);delete f.engineLitres;f.year=Number(f.year);f.price=Number(f.price);if(!Number.isInteger(f.year)||f.year<1900||f.year>2100||!Number.isFinite(f.price)||f.price<=0){notify('Use a year from 1900–2100 and a price above RM0.');return}f.media=mediaDraft.map(m=>({...m}));f.img=f.media.find(m=>m.type==='photo')?.url||'';const idx=data.cars.findIndex(c=>c.id===id);if(idx<0)data.cars.push({id,...f});else data.cars[idx]={...data.cars[idx],...f};}
