...
export const cars=async()=>(await getVehicles()).filter(v=>v.stock_status==='Available'&&v.publication==='published');
...
export const create=vehicle=>db.rpc('e2_create_intake_link',{vehicle:vehicle||null}).then(check).then(v=>Array.isArray(v)?v[0]:v);
...
export const assignVehicle=(a,vehicle)=>db.rpc('e2_intake_assign_vehicle',{target:a.id,vehicle,expected_revision:a.revision}).then(check);
