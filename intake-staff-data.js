...
export {db,check};
export async function who(){const user=check(await db.auth.getUser()).user;if(!user)throw Error('Staff sign in required.');const membership=check(await db.from('staff_memberships').select('role,active').eq('user_id',user.id).single());if(!membership.active||!['super_admin','admin','sales'].includes(membership.role))throw Error('Staff intake access required.');return {...user,role:membership.role}}
