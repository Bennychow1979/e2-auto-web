...
const ids=[photo,p2,p3],arr=values=>`array[${values.map(id=>id===null?'null':`'${id}'`).join(',')}]::uuid[]`;
const reorder=(next,expected)=>`select public.e2_reorder_photos('${car}',${arr(next)},${arr(expected)})`;
for(const [i,id] of [p2,p3].entries())await db.exec(`insert into storage.objects(bucket_id,name) values('vehicle-photos','${car}/${id}.webp');insert into public.vehicle_photos(id,vehicle_id,path,position) values('${id}','${car}','${car}/${id}.webp',${i+1})`);
await db.exec(reorder([p3,photo,p2],ids));
assert.deepEqual((await db.query('select id from public.vehicle_photos order by position')).rows.map(r=>r.id),[p3,photo,p2]);passed++;console.log('PASS Three-photo reorder persists exact order and new cover');
await fails(reorder(ids,ids),'Stale order cannot overwrite a newer arrangement');
await fails(reorder([photo,photo,p3],[p3,photo,p2]),'Reject duplicate photo IDs');
await fails(reorder([photo,p3],[p3,photo,p2]),'Reject missing photo');
await fails(reorder([photo,p3,admin],[p3,photo,p2]),'Reject foreign photo ID');
await fails(reorder([photo,p3,null],[p3,photo,p2]),'Reject null photo ID');
await as(sales);await fails(reorder(ids,[p3,photo,p2]),'Sales cannot rearrange photos');
await as(null);await fails(reorder(ids,[p3,photo,p2]),'Anonymous cannot rearrange photos');
await as(admin);await db.exec(`update public.vehicles set publication='published' where id='${car}'`);
await fails(reorder(ids,[p3,photo,p2]),'Published photos require moving to draft before rearranging');
await db.exec(`update public.vehicles set publication='draft' where id='${car}'`);
await db.exec(reorder(ids,[p3,photo,p2]));
await count(`select count(*) n from public.inventory_audit where action='REORDER'`,2,'Successful reorder is audited; rejected requests are atomic');
for(const id of [p2,p3])await db.exec(`delete from public.vehicle_photos where id='${id}';delete from storage.objects where name='${car}/${id}.webp'`);
