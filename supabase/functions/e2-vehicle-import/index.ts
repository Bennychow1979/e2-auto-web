...
Deno.serve(async req=>{
  ...
  try {
    ...
    const actor=result.data?.user?.id;if(result.error||!actor)return reply(401,{error:'Sign in with an Admin account.'});
    const staff=checked(await admin.from('staff_memberships').select('role,active').eq('user_id',actor).maybeSingle());
    if(!staff?.active||!['super_admin','admin'].includes(staff.role))return reply(403,{error:'Admin access required.'});
    ...
    if(Deno.env.get('E2_VEHICLE_IMPORT_ENABLED')!=='true')return reply(503,{error:'Automatic import is not enabled. MASTERLIST read-only access needs approval and a connection test.'});
    const output=await runImport({drive,store,root,actor,retry:body.retry===true,loadWorkbook:async bytes=>{
  ...
  }catch(error){return reply(400,{error:error instanceof Error?error.message:'Import failed. Refresh before retrying.'})}
});
