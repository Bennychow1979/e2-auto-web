...
const fordModels=["Anglia","Capri","Cortina","Courier","Econovan","Ecosport","Escape","Escort","Everest","Explorer","F150","Falcon","Fiesta","Focus","FREDA","Galaxie","GT","Kuga","Laser","Lynx","Mondeo","Mustang","Prefect","Ranger","Roller Team","S-MAX","Spectron","Telstar","Transit"];
...
const mitsubishiModels=["Airtrek","ASX","Attrage","Challenger","Chariot","Colt","Delica","Dion","Eclipse","FTO","Galant","Grandis","GTO","i-MiEV","Jeep","Lancer","Mirage","Outlander","Pajero","RVR","Shogun","Space Gear","Spacewagon","Storm","Triton","Xforce","Xpander"];
const suzukiModels=["Aerio","Alto","APV","Carry","ERV","Evitara","Fronx","Grand Vitara","Hustler","Ignis","Jimny","Kei","Kizashi","Liana","Palette","Solio","Spacia","Splash","Swift","SX4","Twin","Vitara","Wagon R","X-90"];
const volkswagenModels=["Arteon","Beetle","Bora","Caravelle","CC","Corrado","Cross Touran","Crosspolo","Eos","Golf","ID 3","ID 4","ID 5","ID Buzz","Jetta","Kombi","Passat","Passat CC","Phaeton","Polo","Polo GTi","Scirocco","Scirocco R","Sharan","T-Cross","T-Roc","Tayron","Tiguan","Touareg","Touran","Transporter","Type","Up","Vento"];
const hondaModels=["1300","ACCORD","Airwave","BR-V","CITY","City GM6","City RS","CIVIC","Civic FC","Civic FE","Civic RS","Civic Type R","CR-V","CR-X","CR-Z","Crossroad","E","E:N1","Edix","Elysion","Fit","Freed","HR-V","INSIGHT","Inspire","Integra","Integra Type R","Jade","Jazz","Jazz Hybrid","Lagreat","Legend","Life","MDX","N BOX","N-One","N-Van","NSX","Odyssey","Prelude","Quint","S2000","S600","S660","Shuttle","Stepwagon","Stream","Vigor","WR-V","ZR-V"];
const base=[{brand:'HYUNDAI',model:'GRAND STAREX',variant:''},...hondaModels.map(model=>({brand:'Honda',model,variant:''})),{brand:'Honda',model:'CR-V',variant:'TC-P 2WD'},...bmwModels.map(model=>({brand:'BMW',model,variant:''})),...subaruModels.map(model=>({brand:'SUBARU',model,variant:''})),...fordModels.map(model=>({brand:'FORD',model,variant:''})),...mazdaModels.map(model=>({brand:'MAZDA',model,variant:''})),...mercedesModels.map(model=>({brand:'MERCEDES-BENZ',model,variant:''})),...nissanModels.map(model=>({brand:'NISSAN',model,variant:''})),...peroduaModels.map(model=>({brand:'Perodua',model,variant:''})),...protonModels.map(model=>({brand:'PROTON',model,variant:''})),...toyotaModels.map(model=>({brand:'TOYOTA',model,variant:''})),...mitsubishiModels.map(model=>({brand:'MITSUBISHI',model,variant:''})),...suzukiModels.map(model=>({brand:'SUZUKI',model,variant:''})),...volkswagenModels.map(model=>({brand:'VOLKSWAGEN',model,variant:''}))];
