...
function render(){
  const found=sortedCars(cars.filter(c=>matches(c,state)),sort);
  savePreferences(preferenceStorage,state,sort);
  $('filterResults').innerHTML=found.map(c=>'<a class="sampleRow" href="'+esc(vehicleHref(c.id))+'">'+(c.cover?'<img src="'+esc(c.cover)+'" alt="'+esc(c.brand+' '+c.model)+'" loading="lazy">':'<span class="noPhoto">Photo unavailable</span>')+'<span><span class="sampleLabel">'+esc(c.plate+' · '+c.stock_status)+'</span><h3>'+esc(c.brand+' '+c.model)+'</h3><span class="sampleMeta">'+esc([c.year,c.variant,c.transmission].filter(Boolean).join(' · '))+'<br>'+esc(mileageText(c))+'</span></span><span class="rowMoney">'+rm(c.price)+'<small>Selling price</small></span><span class="rowGo" aria-hidden="true">↗</span></a>').join('');
...
function sync(){$('carSearch').value=state.q;$('budgetFilter').value=state.budget;$('bodyFilter').value=state.body;$('sortFilter').value=sort}
function reset(){state={...blank};draft={...blank};sort='latest';sync();render()}
$('sortFilter').onchange=e=>{sort=e.target.value;render()};
